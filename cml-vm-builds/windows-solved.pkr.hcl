...
source "qemu" "dc01-solved" {
  vm_name = "wsc2024-mod-d-dc01-solved.qcow2"
  accelerator  = "kvm"
  machine_type = "q35"
  cpus         = 4
  memory       = 4096
...
source "qemu" "ws01-solved" {
  vm_name = "wsc2024-mod-d-ws01-solved.qcow2"
  accelerator  = "kvm"
  machine_type = "q35"
  cpus         = 4
  memory       = 4096
...
build {
  sources = ["source.qemu.dc01-solved", "source.qemu.ws01-solved"]

  provisioner "powershell" {
    only   = ["qemu.dc01-solved"]
    use_pwsh = true
    inline = ["Rename-Computer -NewName dc01"]
  }
  
  provisioner "powershell" {
    only   = ["qemu.ws01-solved"]
    use_pwsh = true
  ...
  provisioner "powershell" {
    only   = ["qemu.dc01-solved"]
    use_pwsh = true
    script   = "scripts/winsrv/provision-clean-dc01.ps1"
  }
  ...
  provisioner "powershell" {
    only   = ["qemu.ws01-solved"]
    script   = "scripts/win11/provision-clean-ws01.ps1"
  }
